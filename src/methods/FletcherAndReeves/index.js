  ...
  new Promise((resolve, reject) => {
    ...
    while (norma(g, escopo) > e) {
      ...
      for (let k = 1; k <= n; k++) {
        ...
        //A) xk+1 = xk + L*dk
        for (let i = 1; i <= n; i++) {
          //substituir cada componente de x
          aux = "x" + i;
          f2 = f2.replace(
            new RegExp(aux, "gi"),
            "(" + x[i - 1].toString() + "+x*" + d[i - 1] + ")"
          );
        }
        //console.log("f2: "+f2);
        lambda = newton(f2, 0); //lambda = min f(xk+lambda*dk)
        ...
        if (k < n) {
          dprox = add(
            gxprox.map((el) => -el),
            multiply(
              multiply(transpose(gxprox), gxprox) / multiply(transpose(gx), gx),
              d
            )
          );
