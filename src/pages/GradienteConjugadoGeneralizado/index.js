...
export default function GradienteConjugadoGeneralizado() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      setResp(false);
      setErr("");
      metodos("GradienteConjugado", values)
        .then((value) => {
  ...
  return (
    <Row style={{ marginLeft: 30, marginRight: 30 }}>
      <Col s={12} m={12} l={12}>
        <h4 style={{ fontWeight: "bold" }}>Gradiente Conjugado Generalizado</h4>
      </Col>
      <Col s={4} m={4} l={4}>
        <CardPanel
          name="formulario"
        ...
        >
          <form onSubmit={formik.handleSubmit}>
            <Row style={{ textAlign: "center" }}>
              ...
              <button
                class="btn btn-large red darken-1"
                type="submit"
                name="action"
              >
                Calcular mínimo
        ...
        >
          <Row>
            <div class="col s12 m12 l12">
              <CardPanel className="red darken-1">
                <p style={{ fontSize: 24 }}>
                  {resp ? (
